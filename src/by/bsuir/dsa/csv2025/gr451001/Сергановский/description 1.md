...
Вам дан ориентированный граф, моделирующий зависимости между задачами в проекте. 
Каждая вершина представляет задачу, а ребро `u → v` означает, что задача `u` должна быть выполнена раньше задачи `v`.

## Требуется:
1. Реализовать алгоритм топологической сортировки двумя способами:
   - с использованием обхода в глубину (DFS);
   - с использованием подсчёта входящих степеней (алгоритм Кана).
2. Проверить граф на наличие циклов:
   - если граф содержит цикл, вывести сообщение:  
     **"Топологическая сортировка невозможна: граф содержит цикл."**
3. Если граф ацикличен:
   - вывести корректный топологический порядок;
   - сравнить порядок, полученный DFS, с порядком по алгоритму Кана (они могут различаться).
