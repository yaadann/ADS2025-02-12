...
Вам дан ориентированный граф, представляющий зависимости между модулями программной системы. Каждая вершина графа соответствует модулю, а направленное ребро u → v означает, что модуль u должен быть загружен/обработан до модуля v.

## Задание:

1. Реализовать алгоритм топологической сортировки двумя методами:
   - На основе поиска в глубину (DFS)
   - На основе алгоритма Кана (с использованием подсчёта входящих степеней вершин)

2. Обнаружение циклов:
   - Если граф содержит цикл, вывести сообщение:  
     "Graph contains cycle, topological sorting impossible"

3. Для ациклического графа:
   - Вывести корректный топологический порядок для каждого алгоритма
   - Сравнить результаты двух методов (порядки могут отличаться)
