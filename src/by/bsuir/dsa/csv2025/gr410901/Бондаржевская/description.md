...
We define three functions (in terms of array indices):

```java
void add(idx);         // add element idx to the window
void remove(idx);      // remove element idx from the window
int get_answer();      // extract the current answer from the data structure
```

Example query structure and main loop (in pseudocode):
...
Initially, the window [cur_l, cur_r] is empty, and it expands or contracts as we move on to handle each query.
