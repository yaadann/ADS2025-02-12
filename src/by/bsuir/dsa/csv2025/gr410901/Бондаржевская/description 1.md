...
Зададим три функции (в терминах индекса массива):

```java
void add(idx);         // добавить элемент idx в окно
void remove(idx);      // убрать элемент idx из окна
int get_answer();      // получить текущий ответ из структуры
```

Пример структуры запроса и основного цикла (на псевдокоде):
...
Перед началом окно [cur_l, cur_r] полупустое, и расширяется/сужается по мере движения к каждому запросу.
