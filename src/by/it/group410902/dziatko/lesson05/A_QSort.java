package by.it.group410902.dziatko.lesson05;
...
public class A_QSort {
    ...
    int[] getAccessory(InputStream stream) throws FileNotFoundException {
        ...
        for(int i = 0; i < m;i++) {
            for (Segment seg : segments) {
                if(points[i] >= seg.start && points[i] <= seg.stop) result[i] += 1;
            }
        }
    ...
    int partition(Segment[] arr, int low, int high) {
        int pivot = arr[high].start;// Choosing the last element as pivot
