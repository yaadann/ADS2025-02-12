package by.it.group410902.habrukovich.lesson02;
/*
Даны
1) объем рюкзака 4
...
public class C_GreedyKnapsack {
    ...
    double calc(InputStream inputStream) throws FileNotFoundException {
        ...
        for (Item item : items) {
            if (currentWeight + item.weight <= W) { // берем весь предмет
                result += item.cost;
                currentWeight += item.weight;
            } else { // берем часть предмета
                double fraction = (W - currentWeight) / (double) item.weight;
