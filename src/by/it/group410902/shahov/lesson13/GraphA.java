package by.it.group410902.shahov.lesson13;

import java.util.*;

public class GraphA {
    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in);
        String input = scanner.nextLine();
        Map<String, List<String>> graph = parseGraph(input);
        List<String> result = topologicalSort(graph);
        System.out.println(String.join(" ", result));
        scanner.close();
    }

    // Парсит строку входных данных и строит граф в виде списка смежности
    private static Map<String, List<String>> parseGraph(String input) {
        Map<String, List<String>> graph = new HashMap<>();
        String[] edges = input.split(",\\s*"); // Разделяем по запятым с пробелами
        for (String edge : edges) {
            String[] vertices = edge.split("\\s*->\\s*"); // Разделяем по стрелке
            String from = vertices[0].trim(); // Вершина-источник
            String to = vertices[1].trim();   // Вершина-назначение
            // Добавляем ребро from -> to в граф
            graph.computeIfAbsent(from, k -> new ArrayList<>()).add(to);
            // Гарантируем, что вершина 'to' тоже присутствует в графе (может быть без исходящих ребер)
            graph.computeIfAbsent(to, k -> new ArrayList<>());
        }
        return graph;
    }

    // Выполняет топологическую сортировку графа
    private static List<String> topologicalSort(Map<String, List<String>> graph) {
        Set<String> visited = new HashSet<>(); // Множество посещенных вершин
        List<String> result = new ArrayList<>(); // Результат топологической сортировки
        PriorityQueue<String> queue = new PriorityQueue<>(); // Очередь с приоритетом для лексикографического порядка

        // Находим вершины без входящих ребер (источники)
        Set<String> noIncoming = new HashSet<>(graph.keySet());
        for (List<String> neighbors : graph.values()) {
            // Убираем все вершины, которые являются назначениями каких-либо ребер
            noIncoming.removeAll(neighbors);
        }
        queue.addAll(noIncoming); // Добавляем все источники в очередь

        // Алгоритм Кана для топологической сортировки
        while (!queue.isEmpty()) {
            String vertex = queue.poll(); // Извлекаем вершину с наименьшим лексикографическим порядком
            if (!visited.contains(vertex)) {
                visited.add(vertex);
                result.add(vertex); // Добавляем вершину в результат

                // Обрабатываем всех соседей текущей вершины
                List<String> neighbors = graph.getOrDefault(vertex, new ArrayList<>());
                for (String neighbor : neighbors) {
                    if (!visited.contains(neighbor)) {
                        queue.offer(neighbor); // Добавляем соседа в очередь
                    }
                }
            }
        }

        return result;
    }
}
