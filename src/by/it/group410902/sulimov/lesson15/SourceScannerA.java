package by.it.group410902.sulimov.lesson15;
...
public class SourceScannerA {
    ...
    static class FileInfo implements Comparable<FileInfo> {
        ...
        public int compareTo(FileInfo file) {
            if (this.size != file.size) {
                return Integer.compare(this.size, file.size);
            }
            return this.relativePath.compareTo(file.relativePath);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        List<FileInfo> fileInfs = new ArrayList<>();

        try (Stream<Path> walk = Files.walk(rootPath)) {
            walk.filter(p -> p.toString().endsWith(".java")).forEach(p -> {
                try {
                    String content = readFileIgnorErrors(p);
                    ...
                    fileInfs.add(new FileInfo(relativePath, size));
                } catch (IOException e) { }});
        } catch (IOException e) {
            e.printStackTrace();
        }
        fileInfs.sort(null);
        for (FileInfo info : fileInfs) {
            System.out.println(info.size + " " + info.relativePath);
        }
    }

    private static String readFileIgnorErrors(Path path) throws IOException {
