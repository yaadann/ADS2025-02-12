...
public class B_Huffman {
    ...
    String decode(InputStream inputStream) throws FileNotFoundException {
        ...
        // Сохраним соответствие кодов символам
        Map<String, Character> codeToChar = new HashMap<>();
        for (int i = 0; i < count; i++) {
            String line = scanner.nextLine();
            String[] parts = line.split(": ");
            char letter = parts[0].charAt(0); // символ
            String code = parts[1]; // код
            codeToChar.put(code, letter); // добавляем в карту
        }
        ...
        for (char c : encodedString.toCharArray()) {
            currentCode += c; // накапливаем символы в текущем коде
            if (codeToChar.containsKey(currentCode)) {
                result.append(codeToChar.get(currentCode)); // добавление символа в результат
