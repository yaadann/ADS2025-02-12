package by.it.group410902.kavtsevich.lesson07;
        ...
        //!!!!!!!!!!!!!!!!!!!!!!!!!     НАЧАЛО ЗАДАЧИ     !!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            for (int i = 0; i <= m; i++)
            {
                for (int j = 0; j <= n; j++)
                {
                    if (i == 0)
                    {
                        dp[i][j] = j; // Если первая строка пустая, количество операций — длина второй строки
                    } else if (j == 0)
                    {
                        dp[i][j] = i; // Если вторая строка пустая, количество операций — длина первой строки
                    } else {
