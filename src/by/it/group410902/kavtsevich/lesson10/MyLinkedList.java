...
public class MyLinkedList<E> implements Deque<E>{ //двусвязный список
    ...
    public E poll() { // Извлечение и удаление первого элемента
        if(this.head != null) {
            ...
            if(re_val.next != null) {
                // Если есть следующий элемент, обновляем ссылки
                this.head.next.behind = null;
                this.head = this.head.next;
            }else{
                // Если это последний элемент, очищаем список
                this.head = null;
                this.tail = null;
            }
            return re_val.value;
    ...
    public E remove(int ind) { // Удаление элемента по индексу
        ...
        if(this.head != null && ind < size && ind >=0){
            ...
            // Перенаправляем ссылки вокруг удаляемого элемента
