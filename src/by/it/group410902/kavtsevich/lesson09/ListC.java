package by.it.group410902.kavtsevich.lesson09;
...
public class ListC<E> implements List<E> {
    ...
    // Конструктор со стандартной емкостью
    private E[] el;
    private int size;
    public ListC() {
    ...
    @Override
    public E set(int index, E element) { //замена элемента
    ...
    @Override
    public E get(int index) { //получение
        if (index < 0 || index >= size) throw new IndexOutOfBoundsException();
        return el[index];
    }
    ...
    public boolean contains(Object o) {
        return indexOf(o)>=0; //проверка на наличие
    }
    ...
    @Override
    public boolean containsAll(Collection<?> c) { //проверка всех элементов коллекции
    ...
    public boolean addAll(Collection<? extends E> c) {
        boolean f=false;
        for (E e: c){ //добавление всех элементов
            add(e);
            f=true;
        }
        return f;
    ...
    @Override
    public boolean addAll(int index, Collection<? extends E> c) { //добавление с позиции
        if (index < 0 || index > size) throw new IndexOutOfBoundsException();
        boolean f = false;
        for (E e : c) {
            add(index++, e);
    ...
    @Override
    public boolean removeAll(Collection<?> c) { //удаление указанных элементов
        boolean f = false;
        for (int i = 0; i < size; ) {
            if (c.contains(el[i])) {
    ...
    @Override
    public boolean retainAll(Collection<?> c) {  //оставить только те которые в коллекции
        boolean f = false;
        for (int i = 0; i < size; ) {
