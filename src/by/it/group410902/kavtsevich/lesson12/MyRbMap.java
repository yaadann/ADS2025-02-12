...
public class MyRbMap implements SortedMap<Integer, String> {
    ...
    // Цвета узлов для красно-черного дерева
    private static final boolean RED = true;    // красный цвет узла
    private static final boolean BLACK = false; // черный цвет узла

    private Node root;    // корневой узел дерева, начальная точка всех операций
    private int size;     // количество элементов (пар ключ-значение) в карте

    private class Node {
        Integer key;      // ключ элемента, используется для упорядочивания в дереве
        String value;    // значение элемента, ассоциированное с ключом
        Node left;       // ссылка на левого потомка (узлы с меньшими ключами)
        Node right;      // ссылка на правого потомка (узлы с большими ключами)
        boolean color;   // цвет узла: RED (true) или BLACK (false)
