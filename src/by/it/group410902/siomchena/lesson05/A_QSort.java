...
public class A_QSort {
    ...
    int[] getAccessory(InputStream stream) {
        ...
        for (int i = 0; i < m; i++) {
            int point = points[i];
            int startCount = upperBound(starts, point); // отрезки, начавшиеся до или в этой точке
            int endCount = lowerBound(ends, point);     // отрезки, закончившиеся до этой точки
    ...
    // находит количество элементов <= value
