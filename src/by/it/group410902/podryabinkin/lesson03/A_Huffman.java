package by.it.group410902.podryabinkin.lesson03;
...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        for (Map.Entry<Character, Integer> count_e : count.entrySet()) {
            Character ch = count_e.getKey();
            Integer fr = count_e.getValue();
            priorityQueue.add(new LeafNode(fr,ch));
        }
        ...
        if (root != null) {
            if (count.size() == 1) { // только 1 символ в дереве
                root.fillCodes("0");
            }
            else {
                root.fillCodes(""); //кодируем по нормальному
            }
        }
        ...
        System.out.println(count.size() + " " + sb.length());
