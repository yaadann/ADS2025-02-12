...
public class A_QSort {
    ...
    int[] getAccessory(InputStream stream) throws FileNotFoundException {
        ...
        for (int i = 0; i < m; i++) {
            int point = points[i];
            int count = 0;
            // Проверяем каждый отрезок
            for (Segment segment : segments) {
                // Если начало отрезка больше точки, дальнейшие отрезки тоже не содержат точку
                if (segment.start > point) {
                    break;
                }
                // Проверяем, лежит ли точка внутри отрезка (включая границы)
