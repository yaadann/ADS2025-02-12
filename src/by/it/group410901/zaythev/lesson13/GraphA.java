...
public class GraphA {
    ...
    private final SortedMap<String, SortedSet<String>> adjacencyList = new TreeMap<>(Collections.
            reverseOrder());
    ...
    public GraphA(String input) {
        ...
        for (var edge : edges) {
            ...
            var from = vertices[0];
            var to = vertices[1];
            // Добавляем ребро в список смежности(есть ли ключ from)
            adjacencyList.computeIfAbsent(from, _1_ -> new TreeSet<>(Collections.
                    reverseOrder())).add(to);
        }
