...
public class SourceScannerC {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Сортируем файлы по относительному пути (лексикографически)
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = i + 1; j < n; j++) {
                ...
                // Если длины слишком разные — даже нет смысла сравнивать по Левенштейну
                if (Math.abs(s1.length() - s2.length()) > 9)
                    continue;

                // Дешевый фильтр по hashCode — может отсечь много вариантов быстро
                if (!cheapFilter(s1, s2))
                    continue;

                // Считаем расстояние Левенштейна, но с ограничением max=9
    ...
    /**
     * Читает файл, очищает текст и добавляет в список items.
     */
    private static void processFile(Path file, Path root, List<Item> items) {
        ...
        try {
            ...
            return; // если файл не в UTF-8 — пропускаем
        } catch (IOException e) {
            return; // если ошибка чтения — тоже пропускаем
        }
    ...
    /**
     * Удаляет строки вида "package ...;" и "import ...;".
     */
    ...
    /**
     * Заменяет все символы с кодом <33 (пробельные, табы, CR, LF)
     * на один пробел, гарантируя отсутствие повторов пробелов.
     */
    ...
    /**
     * Ограничивает длину строки 2000 символами.
     * Огромные файлы сильно замедляют Левенштейн — это защита.
     */
    ...
    /**
     * Быстрая проверка по разнице hashCode двух строк.
     * Если hash сильно отличается — тексты почти точно разные.
     */
     ...
     * но с ограничением maxDist.
     * Если расстояние > maxDist — прекращает вычисления
     * и возвращает число > maxDist.
     *
