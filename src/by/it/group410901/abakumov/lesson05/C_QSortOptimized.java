...
public class C_QSortOptimized {
    ...
    int[] getAccessory2(InputStream stream) throws FileNotFoundException {
        ...
        quickSort3WayTailRecursive(segments, 0, segments.length - 1);
    ...
    private int countSegmentsContainingPoint(Segment[] segments, int point) {
        ...
        // Бинарный поиск первого отрезка, чей start <= point
        while (left <= right) {
    ...
    private void quickSort3WayTailRecursive(Segment[] arr, int low, int high) {
        while (low < high) {
            ...
            // Рекурсивно вызываем меньшую часть, чтобы избежать переполнения стека
            if ((lt - low) < (high - gt)) {
                quickSort3WayTailRecursive(arr, low, lt - 1);
                low = gt + 1;  // хвостовая рекурсия
            } else {
                quickSort3WayTailRecursive(arr, gt + 1, high);
                high = lt - 1; // хвостовая рекурсия
            }
        }
    }
