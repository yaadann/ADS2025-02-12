package by.it.group410901.danilova.lesson05;
...
public class C_QSortOptimized {
    ...
    private int countSegmentsContainingPoint(Segment[] segments, int point) {
        int count = 0;
        int firstIndex = findFirstSegmentIndex(segments, point);

        if (firstIndex != -1) {
            // Нашли хотя бы один отрезок, теперь ищем все подходящие
            count = 1;
            // Проверяем отрезки слева от найденного
            for (int i = firstIndex - 1; i >= 0 && segments[i].start <= point; i--) {
    ...
    private int findFirstSegmentIndex(Segment[] segments, int point) {
        int left = 0;
        ...
        while (left <= right) {
            ...
            if (segments[mid].start <= point) {
                if (segments[mid].stop >= point) {
                    result = mid;
                    right = mid - 1; // Ищем самый левый подходящий отрезок
                } else {
                    left = mid + 1;
                }
            } else {
