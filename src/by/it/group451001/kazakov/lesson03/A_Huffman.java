package by.it.group451001.kazakov.lesson03;
...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        count.forEach((Key, Value) -> {
            LeafNode newNode = new LeafNode(Value, Key);
            priorityQueue.add(newNode);
        });

        while (priorityQueue.size() > 1) {
            InternalNode parentNode = new InternalNode(priorityQueue.poll(), priorityQueue.poll());
        ...
        Node finall = priorityQueue.poll();
        finall.fillCodes("");
