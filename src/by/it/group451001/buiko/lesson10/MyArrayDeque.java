...
public class MyArrayDeque<E> implements Deque<E> {
    ...
    // Индекс первого элемента в деке
    private int head;

    // Индекс следующей свободной позиции после последнего элемента
    private int tail;
     ...
     * Инициализирует массив начального размера
     */
     ...
     * Копирует все элементы в новый массив большего размера
     */
    @SuppressWarnings("unchecked")
    private void resize() {
        ...
        // Копируем элементы, начиная с head, в начало нового массива
        int current = head;
        for (int i = 0; i < size; i++) {
            newElements[i] = elements[current];
            // Переходим к следующему элементу с учетом цикличности
            current = (current + 1) % elements.length;
        }

        elements = newElements;
        // После копирования head становится 0, tail - на позиции после последнего элемента
        head = 0;
        tail = size;
    }
