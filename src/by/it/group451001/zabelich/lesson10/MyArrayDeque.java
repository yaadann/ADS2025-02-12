...
public class MyArrayDeque<E> implements Deque<E> {
    ...
    // Дек - это двусторонняя очередь, которая поддерживает добавление/удаление с
    // обоих концов
    private E[] elements;
    ...
    public String toString() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            // % - оператор modulo, который "заворачивает" индекс когда он достигает конца
            // массива
    ...
    // Возвращает true если элемент успешно добавлен (всегда true для этой
    // реализации)
    // Это стандартное поведение для коллекций в Java
    @Override
    public boolean add(E element) {
        // ИСПРАВЛЕНИЕ: addLast() возвращает void, поэтому не можем возвращать его
        // результат
    ...
    public void addFirst(E element) {
        ...
        // - если head был 0, то после вычисления станет elements.length - 1
        // (циклический переход)
        // + elements.length нужно чтобы избежать отрицательных чисел при операции
        // modulo
    ...
    public E getFirst() {
        ...
        if (size == 0) {
            // Бросаем исключение NoSuchElementException как требует контракт интерфейса
            // Deque
    ...
    public E getLast() {
        ...
        // (tail - 1 + elements.length) % elements.length находит индекс последнего
        // добавленного элемента
        // tail указывает на СЛЕДУЮЩУЮ позицию для добавления, поэтому последний элемент
        // на tail-1
    ...
    // Возвращает null если дек пуст (в отличие от removeFirst который бросает
    // исключение)
    @Override
    public E poll() {
    ...
    // Создает новый массив в 2 раза больше и копирует все элементы в правильном
    // порядке
    @SuppressWarnings("unchecked")
    private void resize() {
