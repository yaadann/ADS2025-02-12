package by.it.group451001.ivanov.lesson03;
...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        // priorityQueue.addAll((Collection<? extends Node>) count.entrySet());
        //3. вынимая по два узла из очереди (для сборки родителя)
        //и возвращая этого родитеchля обратно в очередь
        //построим дерево кодирования Хаффмана.
        //У родителя частоты детей складываются.
        Node firstNode, secondNode;
        while (priorityQueue.size() > 1){
            firstNode = priorityQueue.poll();
            secondNode = priorityQueue.poll();
            InternalNode parentNode = new InternalNode(firstNode, secondNode);
            priorityQueue.add(parentNode);
        }
        InternalNode parent = (InternalNode) priorityQueue.poll();
        //4. последний из родителей будет корнем этого дерева
        //это будет последний и единственный элемент оставшийся в очереди priorityQueue.
        String str = "";
        parent.fillCodes(str);
