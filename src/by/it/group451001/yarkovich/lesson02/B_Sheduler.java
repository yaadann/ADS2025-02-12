...
public class B_Sheduler {
    ...
    List<Event> calcStartTimes(Event[] events, int from, int to) {
        ...
        for (Event event : events) {
            if (event.start >= currentTime && event.stop <= to) {  // Если событие начинается не раньше текущего времени и попадает в заданный интервал
                result.add(event);  // Добавляем событие в расписание
                currentTime = event.stop;  // Обновляем текущее время на конец текущего события
            }
        }
