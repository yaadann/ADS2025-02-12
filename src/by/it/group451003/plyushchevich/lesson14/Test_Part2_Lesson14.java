...
public class Test_Part2_Lesson14 extends HomeWork {
    ...
    private String generatePointInput(List<int[]> points) {
        ...
        out.append(n);
        for (int i = 0; i < n; i++) {
            // заменено: random.nextInt(-diapason, diapason)
            int x = random.nextInt(2 * diapason) - diapason; // равносильно [-diapason, diapason)
            int y = random.nextInt(diapason); // [0, diapason-1] как в оригинале
            int z = random.nextInt(diapason);
            int[] point = {x, y, z};
            out.append('\n')
    ...
    private String generateSiteInput(Set<String> sites) {
        ...
        for (int i = 0; i < pairCount*2; i++) {
            // исправлено: индекс для zones должен быть в пределах zones.size()
            String site = words.get(random.nextInt(words.size())) + "." + zones.get(random.nextInt(zones.size()));
            sites.add(site);
