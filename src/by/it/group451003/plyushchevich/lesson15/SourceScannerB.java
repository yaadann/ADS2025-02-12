    ...
    public static void main(String[] args) {
        ...
        while (!stack.isEmpty()) {
            ...
            try (DirectoryStream<Path> stream = Files.newDirectoryStream(dir)) {
                for (Path entry : stream) {
                    try {
                        if (Files.isDirectory(entry)) {
                                ...
                                while ((line = br.readLine()) != null) {
                                    ...
                                    while (i < line.length()) {
                                        if (!inBlockComment && i + 1 < line.length() && line.charAt(i) == '/' && line.charAt(i + 1) == '/') {
                                            break; // однострочный комментарий
                                    ...
                                    String processed = cleanLine.toString().trim();
                            ...
                            if (start >= end) cleaned = ""; else cleaned = cleaned.substring(start, end);
                ...
                // пропускаем проблемную директорию
            }
        }
