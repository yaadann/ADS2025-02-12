...
public class C_GetInversions {
    ...
    static int mergeHalves(int[] arr, final int FROM, final int MID, final int TO){
        int invCount = 0;
    ...
    static int findInv(int[] arr, final int FROM, final int TO){
        int invCount = 0;
        if (FROM < TO) {
            int mid = FROM + (TO - FROM) / 2;
            invCount += findInv(arr, FROM, mid);
            invCount += findInv(arr, mid + 1, TO);
            invCount += mergeHalves(arr, FROM, mid, TO);
        }
        return invCount;
    }
