...
public class SourceScannerC {
    ...
    public static void main(String[] args) {
        ...
        List<by.it.group451003.bernat.lesson15.SourceScannerC.FileData> fileDataList = new ArrayList<>();
    ...
    private static void processJavaFile(Path file, Path srcPath, List<by.it.group451003.bernat.lesson15.SourceScannerC.FileData> fileDataList) {
        try {
            ...
            fileDataList.add(new by.it.group451003.bernat.lesson15.SourceScannerC.FileData(relativePath, processed));
    ...
    private static Map<String, List<String>> findCopiesByHashing(List<by.it.group451003.bernat.lesson15.SourceScannerC.FileData> fileDataList) {
        Map<String, List<String>> copies = new HashMap<>();
        Map<by.it.group451003.bernat.lesson15.SourceScannerC.FileSignature, List<by.it.group451003.bernat.lesson15.SourceScannerC.FileData>> groups = new HashMap<>();

        for (by.it.group451003.bernat.lesson15.SourceScannerC.FileData file : fileDataList) {
            by.it.group451003.bernat.lesson15.SourceScannerC.FileSignature signature = new by.it.group451003.bernat.lesson15.SourceScannerC.FileSignature(file.content);
            groups.computeIfAbsent(signature, k -> new ArrayList<>()).add(file);
        }

        for (List<by.it.group451003.bernat.lesson15.SourceScannerC.FileData> group : groups.values()) {
            if (group.size() > 1) {
                for (int i = 0; i < group.size(); i++) {
                    by.it.group451003.bernat.lesson15.SourceScannerC.FileData file1 = group.get(i);
                    List<String> fileCopies = new ArrayList<>();

                    for (int j = i + 1; j < group.size(); j++) {
                        by.it.group451003.bernat.lesson15.SourceScannerC.FileData file2 = group.get(j);
    ...
    private static class FileSignature {
        ...
        public boolean equals(Object o) {
            ...
            by.it.group451003.bernat.lesson15.SourceScannerC.FileSignature that = (by.it.group451003.bernat.lesson15.SourceScannerC.FileSignature) o;
