...
public class A_Huffman {
    ...
    private class LeafNode extends Node {
        ...
        String encode(InputStream inputStream) throws FileNotFoundException {
            ...
            // 2. Создаем очередь с листовыми узлами
            PriorityQueue<Node> priorityQueue = new PriorityQueue<>();
            ...
            while (priorityQueue.size() > 1) {
                ...
                InternalNode parent = new InternalNode(node1, node2);
                priorityQueue.add(parent);
            }

            // 4. Генерация кодов
            Node root = priorityQueue.poll();
            root.fillCodes(""); // рекурсивно присвоить коды всем листьям

            // 5. Формируем закодированную строку
            StringBuilder encodedStr = new StringBuilder();
            for (char c : s.toCharArray()) {
                encodedStr.append(codes.get(c));
            }

            // Выводим размеры и коды
            // В основном методе main вывод осуществляется уже после вызова encode
            // поэтому здесь только возвращаем закодированную строку
