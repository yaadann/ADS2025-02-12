...
public class B_Huffman {
    ...
    String decode(InputStream inputStream) throws FileNotFoundException {
        ...
        //* Создаем карту для хранения кодов символов (code -> character)
        Map<String, Character> codeMap = new HashMap<>();

        //* Читаем k строк с кодами символов
        for (int i = 0; i < count; i++) {
            String line = scanner.next(); //* Читаем "letter:"
            char letter = line.charAt(0); //* Получаем символ
            scanner.next();               //* Пропускаем ":"
            String code = scanner.next(); //* Читаем код
