...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        // Если считываемая строка пустая, то возвращаем пустой результат
        if (s.isEmpty()) return "";

        // Создаем структуру данных Map
        Map<Character, Integer> count = new HashMap<>();

        // Проходимся по считываемой строке и добавляем ключи и их частоты в count
        ...
        // Переносим символы из count в приоритетную очередь в виде листьев (их частота - приоритет)
        PriorityQueue<Node> priorityQueue = new PriorityQueue<>();
        for (Map.Entry<Character, Integer> entry : count.entrySet())
            priorityQueue.add(new LeafNode(entry.getValue(), entry.getKey()));

        // Вынимаем по два листа, собираем родителя и возвращаем его назад
        while (priorityQueue.size() > 1){
            InternalNode parent = new InternalNode(priorityQueue.poll(), priorityQueue.poll());
            priorityQueue.add(parent);
        }

        // Шифруем строку и возвращаем результат
    ...
    private class InternalNode extends Node {
        ...
        Node left;  //левый потомок бинарного дерева
        Node right; //правый потомок бинарного дерева
