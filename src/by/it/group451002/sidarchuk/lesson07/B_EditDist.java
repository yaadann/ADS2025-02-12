package by.it.group451002.sidarchuk.lesson07;
...
public class B_EditDist {
    ...
    int getDistanceEdinting(String one, String two) {
        ...
        for (int i = 0; i <= m; i++) {
            dp[i][0] = i; // стоимость удаления i символов из первой строки
        }
        for (int j = 0; j <= n; j++) {
            dp[0][j] = j; // стоимость вставки j символов во вторую строку
        }

        // Заполнение матрицы итеративно
        for (int i = 1; i <= m; i++) {
            for (int j = 1; j <= n; j++) {
