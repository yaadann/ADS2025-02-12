package by.it.group451002.spizharnaya.lesson03;
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        for (int i=0;i<=s.length()-1;i++){
            if (count.containsKey(s.charAt(i))){
                count.put(s.charAt(i),count.get(s.charAt(i))+1); //обновить значение по ключу
            }
            else {
                count.put(s.charAt(i),1);                        //добавить пару
            }
        }

        //2. перенесем все символы в приоритетную очередь в виде листьев
        Set<Character> keys = count.keySet();       //коллекция из всех ключей
        PriorityQueue<Node> priorityQueue = new PriorityQueue<>();
        for(char element : keys){                   //для всех ключей
            priorityQueue.add(new LeafNode(count.get(element),element));
        }
        ...
        root.fillCodes("");  //заполнение treemap codes полученными из дерева кодами символов

        //создание сжатой строки str
        StringBuilder str = new StringBuilder();
        for (int i=0;i<=s.length()-1;i++){
            str.append(codes.get(s.charAt(i)));
        }
