package by.it.group451002.spizharnaya.lesson07;
    ...
    private static int editDistTD(int[][] D, int i, int j, String one, String two){
        if (D[i][j] == Integer.MAX_VALUE){
            if (i == 0){
                ...
                //вставка
                int ins = editDistTD(D,i,j-1, one, two)+1;
                //удаление
                int del = editDistTD(D,i-1,j, one, two)+1;
                //(не)совпадение
                int sub = editDistTD(D,i-1,j-1, one, two) + diff(one.charAt(i-1),two.charAt(j-1));
