...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        //рассчитаем частоту каждого символа в строке
        ...
        for (Map.Entry<Character,Integer> entry : count.entrySet()){
            char symbol = entry.getKey();//символ
            int frequency = entry.getValue();//количество повторений
            Node node = new LeafNode(frequency, symbol);//создаем лист
            priorityQueue.add(node);//добавляем лист в очередь
        }

        //создаем дерево кодирования Хаффмана
        while (priorityQueue.size() != 1){
            //вынимаем из очереди два ребенка
            Node left = priorityQueue.poll();//левый ребенок
            Node right = priorityQueue.poll();//правый ребенок
            //заносим в очередь родителя
            priorityQueue.add(new InternalNode(left, right));
        }
        //в итоге в очереди останется один элемент - корневой узел
